...
class HomelabAgent:
    ...
    def __init__(
        ...
        # Memory service for vector-based memory
        self._memory_service = MemoryService(
            db_path=config.runtime_dir / "data" / "memories.db",
        ...
        # Stop flags for interrupting agent runs (per user)
    ...
    async def _handle_message(self, incoming: IncomingMessage) -> str:
        ...
        if content.lower() == "/clear":
            ...
            return "🗑️ Conversation cleared. Let's start fresh!"
        
        if content.lower() == "/stop":
            # Set stop flag for this user
            self._stop_flags[user_id] = True
            # Send stop message to interrupt any ongoing agent run
            # and inform the agent to stop its current task
            content = "stop"
        ...
        try:
            ...
            if isinstance(self._llm_provider, GoogleADKProvider):
                ...
                # Set memory context for memory tools
                set_memory_context(
    ...
    async def _handle_tool_notification(
        ...
        """
        # Skip logging for share_reasoning tool (too verbose)
        if tool_name == "share_reasoning":
            return
        ...
        if result is None:
            ...
            # Send notification with tool name and args
            if chat_id and self._channel:
                try:
                    ...
                    outgoing = OutgoingMessage(
                        content=f"🔧 `{tool_name}`{args_display}",
                        chat_id=chat_id,
                        parse_mode="Markdown",
                    )
                    await self._channel.send_message(outgoing)
