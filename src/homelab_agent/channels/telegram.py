...
class TelegramChannel(BaseCommunicationChannel):
    ...
    async def _handle_forget(
        ...
        if self._forget_callback:
            try:
                ...
                if success:
                    await update.message.reply_text(
                        "🗑️ Memory cleared! Starting fresh conversation."
                    )
                else:
                    await update.message.reply_text(
            ...
            await update.message.reply_text(
                "ℹ️ Session management not available."
            )
    ...
    async def _handle_status(
        ...
        await update.message.reply_text(
            "✅ *Agent Status*\n\n"
            f"🔧 LLM Provider: `{self._config.llm_provider}`\n"
            f"📡 Channel: `{self._config.communication_channel}`\n"
            f"📂 Runtime: `{self._config.runtime_dir}`",
            parse_mode="Markdown",
        )
    ...
    async def _process_bundled_messages(self, chat_id: str) -> None:
        ...
        async with lock:
            ...
            if not self._message_handler:
                ...
                if pending[0].update.message:
                    await pending[0].update.message.reply_text(
                        "⚠️ No message handler configured. Please contact the administrator."
                    )
                return
            ...
            try:
                ...
                # Reply to the last message in the bundle with Markdown
                last_update = pending[-1].update
                if last_update.message:
                    try:
                        await last_update.message.reply_text(response, parse_mode="Markdown")
                    except Exception:
    ...
    async def _handle_voice(
        ...
        if not self._transcription_service:
            await update.message.reply_text(
                f"🎤 *Voice message received!* ({duration}s)\n\n"
        ...
        try:
            ...
            if not result.success:
                await update.message.reply_text(
                    f"🎤 *Voice message received!* ({duration}s)\n\n"
                    f"❌ Could not transcribe: {result.error}",
                    parse_mode="Markdown",
                )
                return
    ...
    async def _handle_photo(
        ...
        if not self._image_service:
            await update.message.reply_text(
                f"📸 *Photo received!* ({photo.width}x{photo.height})\n\n"
        ...
        try:
            ...
            if result.success:
                ...
                await update.message.reply_text(
                    f"📸 Photo received but analysis failed: {result.error}",
                )
                
        except Exception as e:
            logger.exception(f"Error analyzing photo: {e}")
            await update.message.reply_text(
                f"📸 Photo received but I couldn't analyze it: {e}"
            )
    ...
    async def _handle_video(
        ...
        await update.message.reply_text(
            f"🎬 *Video received!* ({video.width}x{video.height}, {duration}s)\n\n"
            f"{f'Caption: {caption}' if caption else 'No caption'}\n\n"
    ...
    async def _handle_video_note(
        ...
        await update.message.reply_text(
            f"⭕ *Video note received!* ({duration}s)\n\n"
